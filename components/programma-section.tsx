...
const saturdayEvents: EventItem[] = [
  ...
  },
  {
    ...
    description:
      "Quale legame unisce la nobile famiglia Dandolo al piccolo paese di Grizzo? Una serata tra storia locale e scoperte sorprendenti. Un'indagine storica che svela connessioni inaspettate tra la grande storia veneziana e le radici del Friuli più autentico.",
...
const sundayEvents: EventItem[] = [
  ...
  },
  {
    ...
    description:
      "L'autore racconta 18+1 itinerari in bicicletta tra Veneto e Friuli. Un libro di viaggio che è anche un libro di incontri, di paesaggi e di storie di chi abita questi territori. Ogni itinerario è una scoperta: borghi dimenticati, osterie nascoste, panorami mozzafiato.",
    author: "Eros Viel",
  ...
  },
  {
    ...
    description:
      "Laboratorio per bambini dai 6 anni in su. Con scarti e pezzetti di carta colorata realizzeremo fantastiche forme, delicate e leggere come petali e piume. Un viaggio creativo nel mondo della carta che stimola la manualità e la fantasia dei piccoli artisti.",
  ...
  },
  {
    ...
    description:
      "Un colorato viaggio tra musica e storie, ritmi coinvolgenti e facili cori da cantare insieme. Laboratorio per bambini dai 4 agli 8 anni che unisce la lettura all'animazione musicale.",
    category: "bambini",
    note: "Iscrizione richiesta",
    location: "Spazio laboratori",
    image: "/images/kids-workshop.jpg",
  },
  {
    ...
    description:
      "Laboratorio gratuito di origami aperto a tutte le età. Impareremo le tecniche base e avanzate dell'arte della piegatura della carta, un'antica tradizione giapponese ricca di creatività.",
  ...
  },
  {
    ...
    description:
      "Una raccolta regionale di racconti e poesie che celebra il confronto culturale tra autrici italiane, slovene e croate. La frontiera come luogo di incontro, scambio e arricchimento reciproco.",
    curator: "Vita Activa Editoria Trieste",
    category: "letterari",
    location: "Sala interna",
    image: "/images/hero-courtyard.jpg",
  },
  {
    ...
    description:
      "Lettura animata in italiano e friulano e un'attività creativa che stimola fantasia e manualità. Laboratorio per bambini dai 4 agli 8 anni che celebra la ricchezza linguistica del Friuli.",
  ...
  },
  {
    ...
    description:
      "Presentazione dell'ultimo libro postumo di Pierluigi Di Piazza da parte di suo fratello Vito. Un incontro commovente dedicato alle parole che hanno guidato una vita spesa al servizio degli ultimi.",
...
function MercatinoCard({ event }: { event: EventItem }) {
  return (
    <Link
      ...
      className="group relative block w-full overflow-hidden rounded-2xl border-2 border-accent/30 bg-gradient-to-br from-accent/10 via-white to-primary/5 shadow-xl transition-all hover:-translate-y-1 hover:shadow-2xl"
    >
      ...
      <div className="relative flex flex-col items-center gap-6 p-8 md:flex-row md:p-10">
        <div className="relative h-40 w-40 flex-shrink-0 overflow-hidden rounded-xl shadow-lg md:h-48 md:w-48">
          <Image
            src={event.image || "/images/books-market.jpg"}
            alt={event.title}
            fill
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <p className="mt-4 text-sm font-semibold text-primary transition-opacity group-hover:opacity-100">
            Scopri di piu →
          </p>
        </div>
      </div>
...
function EventCard({ event }: { event: EventItem }) {
  ...
  return (
    <Link
      href={`/evento/${event.slug}`}
      className="group block w-full rounded-xl border border-border bg-white p-5 text-left shadow-md transition-all hover:-translate-y-0.5 hover:shadow-lg"
    >
      ...
      <h4 className="font-serif text-lg font-bold text-foreground transition-colors group-hover:text-primary">
        {event.title}
      </h4>
      {event.author && (
...
export default function ProgrammaSection() {
  ...
  const showMercatino =
    mercatino &&
    (activeFilter === "all" || activeFilter === mercatino.category);
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="mt-10 flex justify-center">
          <div className="overflow-hidden rounded-xl border border-border shadow-lg">
            <Image
              ...
              alt="Programma completo del festival"
              width={600}
              height={850}
        ...
        <div className="mt-12 flex justify-center gap-4">
          {(["sabato", "domenica"] as const).map((day) => (
            <button
              ...
              className={`rounded-lg px-6 py-3 font-semibold capitalize transition-all ${activeDay === day ? "bg-primary text-primary-foreground shadow-md" : "bg-white text-foreground shadow-sm hover:bg-secondary"}`}
            >
              {day}
            </button>
          ))}
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2">
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all ${activeFilter === "all" ? "bg-foreground text-background" : "bg-white text-foreground shadow-sm hover:bg-secondary"}`}
          >
            Tutti
          </button>
          {(
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-sm font-medium transition-all ${activeFilter === key ? config.color : "bg-white text-foreground shadow-sm hover:bg-secondary"}`}
            >
              {config.icon}
              {config.label}
            </button>
